...
#     curl -sfL https://raw.githubusercontent.com/devpro/infrastructure-provisioning/feature/init-scripts/scripts/setup.sh | GIT_REVISION=refs/heads/develop sh -s -- -o temp
#   Downloading scripts from a specific revision "xxxx" in the default local folder:
#     curl -sfL https://raw.githubusercontent.com/devpro/infrastructure-provisioning/feature/init-scripts/scripts/setup.sh | GIT_REVISION=xxxx sh -
#   Testing locally the setup script:
#     GIT_REVISION=refs/heads/feature/init-scripts ./lab-setup/scripts/setup.sh -o temp
#
...
setup_env() {
  ...
  GIT_REPO_NAME='infrastructure-provisioning'
  GIT_FOLDER=$(echo "$GIT_REVISION" | sed 's/\//-/g' | sed 's/refs-//' | sed 's/heads-//')
}

download() {
  info 'Download scripts'
  wget https://github.com/devpro/${GIT_REPO_NAME}/archive/${GIT_REVISION}.zip -O ${GIT_REPO_NAME}.zip
